...
const std::shared_ptr<bTodo::frontend::TaskCreateModal> task_create_modal{std::make_shared<bTodo::frontend::TaskCreateModal>()};
...
int main() {
  ...
  for(int i = 0; i < 25; i ++){ todo_menu->addTask(i,test_task); }
  ...
  auto settings_menu = Container::Vertical({ bTodo::frontend::components::PhantomComponent(text("Page Has Not Been Constructed") | bgcolor(Color::Red)), });
  ...
  auto renderer = Renderer(application_container, [&] {
    ...
    if(task_create_modal->isActive()) {
      ...
      document = dbox({
        document,
        task_create_modal->getModal()->Render() | clear_under | center
      });
    }else {
      active_layer = 0;
    }
