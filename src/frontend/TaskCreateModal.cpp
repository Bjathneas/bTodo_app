...
namespace bTodo::frontend {
  ftxui::Component TaskCreateModal::createModal() {
    for(int i = 1; i < 32; i++) {
      if(i <= 12)
        this->months.push_back(std::to_string(i));
      if(i >= 24)
        this->years.push_back("20" + std::to_string(i));
      this->days.push_back(std::to_string(i));
    }

    this->modal = ftxui::Container::Vertical({
      ...
      ftxui::Container::Horizontal({
        ...
        ftxui::Input(&this->task_name, "Task Name") | ftxui::bgcolor(ftxui::Color::Grey42) | ftxui::size(ftxui::HEIGHT, ftxui::LESS_THAN, 1)
      }),
      ftxui::Container::Horizontal({
        bTodo::frontend::components::PhantomComponent(ftxui::text("Task Description: ")) | ftxui::vcenter,
        ftxui::Input(&this->task_description, "Task Description") | ftxui::bgcolor(ftxui::Color::Grey42) | ftxui::size(ftxui::HEIGHT, ftxui::LESS_THAN, 10)
      }),
      ftxui::Container::Horizontal({
